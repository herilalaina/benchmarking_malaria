...
if __name__ == "__main__":
    ...
    for i, solution_file in enumerate(solution_names):
        for j, (metric_name, scoring_function) in enumerate(list_metric):
        # metric_name, scoring_function = list_metric[0]
            set_num = i + 1  # 1-indexed
            score_name = 'set%s_score_%s' % (set_num, j)
            ...
            try:
                ...
                try:
                    ...
                    if metric_name in ["accuracy_score", "precision_score", "recall_score"]:
                        score = scoring_function(solution, np.around(prediction))
                    else:
