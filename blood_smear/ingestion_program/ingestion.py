...
if __name__=="__main__":
    ...
    try:
        ...
        from model_for_upload import Model    			 # example model, in scikit-learn style
...
#    datanames = [] # Do not proceed with learning and testing
    ...
    print(D)
    print("[+] Size of uploaded data  %5.2f bytes" % data_io.total_size(D))
    ...
    print("[+] Cumulated time budget (all tasks so far)  %5.2f sec" % (overall_time_budget))
    ...
    start = time.time()
    M_clf.fit(D.data['X_train'], D.data['Y_train'])
    print("[+] Fitting success, time spent so far %5.2f sec" % (time.time() - start))
    ...
    print("[+] Prediction success, time spent so far %5.2f sec" % (time.time() - start))
    # Write results
    # -------------
    ...
    print("[+] Results saved, time spent so far %5.2f sec" % (time.time() - start))
    time_spent = time.time() - start
    ...
    if execution_success:
        print("[+] Done")
        print("[+] Overall time spent %5.2f sec " % overall_time_spent + "::  Overall time budget %5.2f sec" % overall_time_budget)
    else:
        print("[-] Done, but some tasks aborted because time limit exceeded")
        print("[-] Overall time spent %5.2f sec " % overall_time_spent + " > Overall time budget %5.2f sec" % overall_time_budget)
